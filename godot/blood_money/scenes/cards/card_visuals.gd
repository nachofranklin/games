...
@onready var icon_border: PanelContainer = %IconBorder
...
func _update_styleboxes(card: Card) -> void:
	...
	icon_b_sb = icon_pc_sb.duplicate()
	...
	icon_b_sb.draw_center = false
	icon_b_sb.set_border_width_all(border_width)
	icon_b_sb.set_expand_margin_all(border_width - 1)
	
	match card.rarity:
		Card.Rarity.COMMON:
			icon_b_sb.border_color = rarity_colour['COMMON']
		Card.Rarity.UNCOMMON:
			icon_b_sb.border_color = rarity_colour['UNCOMMON']
		Card.Rarity.RARE:
			icon_b_sb.border_color = rarity_colour['RARE']
	...
	icon_border.add_theme_stylebox_override('panel', icon_b_sb)
