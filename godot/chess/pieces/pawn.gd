	...
	for dir in directions: # should be just one for pawns
		...
		if occupier == null:
			...
			if has_never_moved:
				...
				if is_in_bounds(two_forward, board_state): # shouldn't be needed but as board size could be custom and dumb i'll keep this in
					var two_forward_occupier = board_state[two_forward.y][two_forward.x]
					if two_forward_occupier == null:
						moves.append(two_forward)
		
		#elif is_enemy(occupier): # this part doesn't apply as pawns can't take by moving up 1
			#moves.append(new_pos)
			#continue
		#else:
			#continue
		...
		for att_dir in attack_directions:
			...
			if is_in_bounds(att_pos, board_state):
				var att_pos_occupier = board_state[att_pos.y][att_pos.x]
				if att_pos_occupier != null and is_enemy(att_pos_occupier):
