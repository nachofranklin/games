...
func _setup_weight_for_tier(tier: int):
	var battles: Array[BattleStats] = _get_all_battles_for_tier(tier)
	total_weights_by_tier[tier] = 0.0
	
	for battle: BattleStats in battles:
		total_weights_by_tier[tier] += battle.weight
		battle.accumulated_weight = total_weights_by_tier[tier] # every BattleStats will have their unique accumulated weight as a var (not every BattleStats.accumulated_weight will equal the total accumulated weight)
